...
public class ModifyCT extends JFrame implements ItemListener {
	//資料庫JDBC Driver名稱
	static String url = "";
	...
	private JTextField CTName_t, CTChiAbbr_t, CTPhone_t, CTFax_t, CTIndCate_t, CTTaxID_t, 
	                   ...
	                   CTBank_t, CTDAcco_t, CTBankName_t, CTSDelReq_t, other1_t, other2_t, 
	                   build_t, CTAddr_t;
	...
	JCheckBox checkBox_1 = new JCheckBox("\u7576\u670830\u65E5");
		...
		EventQueue.invokeLater(new Runnable() {
			...
			public void run() {
				...
				try {
					...
					final ResultSet rs = statement.executeQuery("SELECT * FROM CT Where CTNum = 31");
					...
					ModifyCT frame = new ModifyCT(null,data,3,"","","");//NewCTData(String ID)
					frame.setVisible(true);
				} catch (Exception e) {
					e.printStackTrace();
				}
	 ...
	 * @param url2 
	 */
	public ModifyCT(final Connection conn, final Object data[],final int EST,final String URL,final String UN,final String PW) {
		setResizable(false);
		con = conn;
		url = URL;
		username = UN;
		...
		ButtonGroup chkbox7 = new ButtonGroup();               //付款條件
				...
				try{
					...
					if(empty ==0){
						...
						if(checkCT(CTName,CTChiAbbr).equals(CTName)){
							...
							System.out.println(CTAddr_t.getText());

							PreviewModifyCT preview = new PreviewModifyCT(con,clone, CTName_t.getText(), CTChiAbbr_t.getText(), CTPhone_t.getText(),
									...
									CTSDelReq_t.getText(), other1_t.getText(), other2_t.getText(), build_t.getText(),
									column, column2, column3, column4, column5, column6, column7, CTAddr_t.getText(),EST,(String)data[30] );
		...
		CTConpName1_t.setText((String) data[12]);
		...
		CTConpName2_t.setText((String) data[13]);
		...
		build_t = new JTextField();
		build_t.setBounds(134, 456, 150, 15);
		contentPane.add(build_t);
		build_t.setColumns(10);
		build_t.setText((String) data[27]);
		
		CTName_t = new JTextField();
		CTName_t.setBounds(113, 56, 115, 15);
		contentPane.add(CTName_t);
		CTName_t.setColumns(10);
		CTName_t.setText((String) data[1]);
		
		CTAddr_t = new JTextField();
		CTAddr_t.setBounds(111, 481, 248, 15);
		contentPane.add(CTAddr_t);
		CTAddr_t.setColumns(10);
		CTAddr_t.setText((String) data[5]);
		//取得被選中之選項
		if(data[16].toString().equals(Own.getText()))
		 {
		...
		if(data[17].toString().equals(checkBox1.getText()))
		 {
			checkBox1.setSelected(true);
		 }else if(data[17].toString().equals(checkBox2.getText())){
			 checkBox2.setSelected(true);
		 }else{
			 checkBox3.setSelected(true);
		 }
		
		if(data[18].toString().equals(checkBox4.getText()))
		 {
			checkBox4.setSelected(true);
		 }else if(data[18].toString().equals(checkBox5.getText())){
			 checkBox5.setSelected(true);
		 }else{
			 checkBox6.setSelected(true);
		 }
		
		if(data[19].toString().equals(checkBox7.getText()))
		 {
			checkBox7.setSelected(true);
		 }else if(data[19].toString().equals(checkBox8.getText())){
			 checkBox8.setSelected(true);
		 }else{
			 checkBox9.setSelected(true);
		 }
		
		if(data[25].toString().equals(checkBox_1.getText()))
		 {
			checkBox_2.setSelected(true);
		 }
		
		if(data[24].toString().equals(checkBox10.getText()))
		 {
			checkBox10.setSelected(true);
		 }else if(data[24].toString().equals(checkBox11.getText())){
			 checkBox11.setSelected(true);
		 }else if(data[24].toString().equals(checkBox11.getText())){
			 checkBox12.setSelected(true);
		 }else{
			 other1_t.setText((String) data[24]);
		 }
			 
		if(data[26].toString().equals(checkBox13.getText()))
		 {
			checkBox13.setSelected(true);
		 }else if(data[26].toString().equals(checkBox14.getText())){
			 checkBox14.setSelected(true);
		 }else if(data[26].toString().equals(checkBox15.getText())){
			 checkBox15.setSelected(true);
		 }else{
			 other2_t.setText((String) data[26]);
		 }
		
		}
	
	public ModifyCT(Object conn, Object data, String string, String uRL2, String uN, String pW) {
	}
	public String checkCT(String CTName2,String CTChiAbbr2){		//取得指定客戶資料
		   Statement statement;
		   ResultSet rs;
