...
public class PreivewmodifyOrder extends JFrame {  //預覽修改訂單
	...
	public PreivewmodifyOrder(Connection conn, Object data[],modifyOrder  nw ,final String OR_Date,final String CTName_ta,final String CTNum_ta,
		...
		/*------------------------------------
		此修改訂單，若OR_checck 為3 表示已出貨，就不能再修改了!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		此修改訂單，若OR_checck 為3 表示已出貨，就不能再修改了!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		此修改訂單，若OR_checck 為3 表示已出貨，就不能再修改了!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		-----------------------------------*/
			...
			comfirmAdd_Btn.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					...
					if(checkstock(pro2,mod2,qua2)==true){
						...
						int isUpdate = 0;	//記錄總更新成功次數
							try {
								...
								if(isUpdate>0){
									...
									JOptionPane.showMessageDialog(PreivewmodifyOrder.this, "修改訂單資料成功 !","修改成功",JOptionPane.INFORMATION_MESSAGE);
									dispose();
									//System.exit(0);
								}else{
									JOptionPane.showMessageDialog(PreivewmodifyOrder.this, "修改訂單資料失敗 !","修改失敗",JOptionPane.WARNING_MESSAGE);
						...
						JOptionPane.showMessageDialog(PreivewmodifyOrder.this, "修改的商品中有庫存不足的情況，請保留紙本訂單","修改失敗",JOptionPane.WARNING_MESSAGE);
						int result=JOptionPane.showConfirmDialog(PreivewmodifyOrder.this,
					               "確定要結束程式嗎?",
					               "確認訊息",
					               JOptionPane.YES_NO_OPTION,
					...
					for(int k=0 ;k<csnn.length;k++){
						if(mod3[i].equals(csnn[k][0])){     //把傳進來的和取出來的找相同型號
							...
							if(qq>ss){
								...
								JOptionPane.showMessageDialog(PreivewmodifyOrder.this,mess,"庫存不夠警告",JOptionPane.WARNING_MESSAGE);
								checkStock++;
							}
						}
					}
			...
			try {
				...
				try{
					...
					if(isUpdate==checkcount){
						...
						JOptionPane.showMessageDialog(PreivewmodifyOrder.this, "更新產品-訂單需求量資料成功 !","新增成功",JOptionPane.INFORMATION_MESSAGE);
						//System.exit(0);
					}else{
						JOptionPane.showMessageDialog(PreivewmodifyOrder.this, "更新產品-訂單需求量資料失敗 !","新增失敗",JOptionPane.WARNING_MESSAGE);
						int result=JOptionPane.showConfirmDialog(PreivewmodifyOrder.this,
					               "確定要結束程式嗎?",
					               "確認訊息",
					               JOptionPane.YES_NO_OPTION,
